...
class DepartmentFactoryOwnersTests(TestCase):
    def test_each_passed_in_user_becomes_an_owner(self):
        alice = UserFactory()
        bob = UserFactory()
    ...
    def test_each_passed_in_user_also_becomes_a_member(self):
        alice = UserFactory()
        bob = UserFactory()
        dept = DepartmentFactory(owners=[alice, bob])
...
class DepartmentFactoryWithUserTests(TestCase):
    def test_true_auto_generates_a_member(self):
        dept = DepartmentFactory(with_user=True)
        self.assertEqual(dept.group.user_set.count(), 1)

    def test_true_does_not_make_the_member_an_owner(self):
        dept = DepartmentFactory(with_user=True)
        member = dept.group.user_set.get()
        self.assertNotIn(member, dept.owners.all())

    def test_specific_user_is_used_as_the_member(self):
        user = UserFactory()
        dept = DepartmentFactory(with_user=user)
        self.assertIn(dept.group, user.groups.all())

    def test_specific_user_is_not_added_as_an_owner(self):
        user = UserFactory()
        dept = DepartmentFactory(with_user=user)
