...
They are also automatically granted the ability to manage FormDefinition objects (so, the schema for forms) and workflows for any department they are the owner of, 
as well as any children of that deparment.
...
- During the invitation process, an administrator or department owner can assign one or both of two department-scoped permissions as defined by `departments.models.DepartmentPermissions`: 
    - CAN_MANAGE_FORMS refer to the `FormDefinition` object from django-forms-workflows, i.e., the ***schema*** of a form in which its fields and behavior are defined.
    - CAN_MANAGE_WORKFLOWS refer to the Approval Workflow object from django-forms-workflows.

- Permissions to view form ***submissions*** cannot be scoped to a department; instead, they are ***always*** defined per user on the FormDefinition, to enforce the principle of least access to that information.
...
## FormAccessService
When a user requests access to a view, that access is managed by FormAccessService, which checks a number of other primitive classes to determine what the user can
or should not have access to, including the FormPermissionService class.