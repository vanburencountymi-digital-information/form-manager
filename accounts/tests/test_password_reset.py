...
class PasswordResetPagesTests(TestCase):
    ...
    def test_submitting_an_unknown_email_does_not_send_an_email(self):
        self.client.post(reverse("password_reset"), {"email": "nobody@example.com"})
        self.assertEqual(len(mail.outbox), 0)

    def test_submitting_redirects_to_the_done_page(self):
...
class PasswordResetFlowTests(TestCase):
    ...
    def test_unusable_password_user_still_gets_a_reset_email(self):
    ...
    def test_invalid_token_does_not_allow_setting_a_new_password(self):
        bad_url = reverse(
