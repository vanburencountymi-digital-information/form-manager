...
class InviteUserFormTests(TestCase):
    ...
    def _assemble_form(self, email: str = "", department=None, user=None, **extra):
        data = {
            "email": email,
            "first_name": "Jane",
            "last_name": "Doe",
        ...
        data.update(extra)
        return InviteUserForm(data=data, user=user if user is not None else self.inviter)
    ...
    def test_saving_adds_the_new_user_correctly(self):
        form = self._assemble_form(email="jane@example.com")
        self.assertTrue(form.is_valid(), form.errors)
        user = form.save()
    ...
    def test_non_administrator_cannot_submit_a_department_they_do_not_own(self):
        other_department = DepartmentFactory(name="Sales")
        form = self._assemble_form(email="jane@example.com", department=other_department)
        self.assertFalse(form.is_valid())
        self.assertIn("department", form.errors)

    def test_non_administrator_can_submit_a_department_they_own(self):
...
class GetDepartmentFieldQuerysetTests(TestCase):
    ...
    def test_returns_empty_queryset_for_none_user(self):
    ...
    def test_excludes_departments_the_user_does_not_own(self):
        other = DepartmentFactory(name="Sales")
        result = self.form.get_department_field_queryset(self.owner)
        self.assertNotIn(other, result)

    def test_includes_descendants_of_owned_departments(self):
        child = DepartmentFactory(name="Backend", parent=self.department)
        result = self.form.get_department_field_queryset(self.owner)
        self.assertIn(child, result)

    def test_excludes_archived_departments_for_non_administrator(self):
        self.department.archive()
        result = self.form.get_department_field_queryset(self.owner)
        self.assertNotIn(self.department, result)

    def test_returns_all_departments_for_administrator(self):
    ...
    def test_excludes_archived_departments_for_administrator(self):
        admin_user = UserFactory(is_administrator=True)
        self.department.archive()
        result = self.form.get_department_field_queryset(admin_user)
        self.assertNotIn(self.department, result)
...
class HandleIsAdministratorFieldTests(TestCase):
    ...
    def test_removes_the_field_for_a_non_administrator(self):
    ...
    def test_removes_the_field_for_a_none_user(self):
        admin_user = UserFactory(is_administrator=True)
        form = InviteUserForm(user=admin_user)
    ...
    def test_safe_to_call_more_than_once_for_a_non_administrator(self):
