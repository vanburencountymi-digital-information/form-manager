...
class InviteUserViewTests(TestCase):
    ...
    def test_user_with_no_department_and_not_administrator_is_denied(self):
        user = UserFactory()
        self.client.force_login(user)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 403)

    def test_department_owner_can_access_the_page(self):
        self.client.force_login(self.owner)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)

    def test_administrator_with_no_owned_department_can_access_the_page(self):
        admin_user = UserFactory(is_administrator=True)
        self.client.force_login(admin_user)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)

    def test_owner_post_creates_user_and_adds_to_department(self):
        self.client.force_login(self.owner)
    ...
    def test_invalid_post_does_not_create_a_user_and_re_renders(self):
        self.client.force_login(self.owner)
    ...
    def test_owner_cannot_grant_administrator_via_tampered_post(self):
        self.client.force_login(self.owner)
    ...
    def test_administrator_post_without_flag_does_not_grant_administrator(self):
        admin_user = UserFactory(is_administrator=True)
    ...
    def test_administrator_post_with_flag_grants_administrator(self):
        admin_user = UserFactory(is_administrator=True)
        self.department.owners.add(admin_user)
        self.client.force_login(admin_user)
        self.client.post(self.url, self._post_data(is_administrator="on"))
